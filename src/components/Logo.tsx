    ...
    const interval = setInterval(() => {
      ...
      if (type < 0.5) {
        ...
        Object.assign(particle.style, {
          ...
          zIndex: "10",
          opacity: "0",
          boxShadow: `0 0 ${pSize * 3}px rgba(255,${Math.floor(Math.random() * 80 + 100)},0,0.7)`,
        });
      } else {
        // heat wisps
        ...
        Object.assign(particle.style, {
          ...
          height: `${pSize * 1.3}px`,
          borderRadius: "50%",
          background: `radial-gradient(circle, rgba(255,${Math.floor(Math.random() * 60 + 40)},0,0.35), transparent)`,
          pointerEvents: "none",
          zIndex: "10",
          opacity: "0",
      ...
      particle.style.animation = `logo-particle ${duration}s ease-out forwards`;
  ...
  if (!flames) {
    return (
      <img
        src="/logo.png"
        alt="ROAST3D"
        style={{ height: h, width: "auto", display: "inline-block", borderRadius: 10 }}
      />
    );
  }
    ...
    >
      <style>{`
        @keyframes logo-particle {
          ...
          15% { opacity: 1; }
          100% { opacity: 0; transform: translateY(-${h * 0.7}px) translateX(var(--drift, 0px)) scale(0.2); }
        }
